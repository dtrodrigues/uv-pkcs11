...
is inspected. For every identity it finds it prints the URI that names that
identity, ready to paste into `SSL_CLIENT_CERT`. The exit status is 0 when
exactly one identity would be selected.
...
With several identities it prints `AMBIGUOUS` and one URI per identity, so
you can pass back the one you want; with none it prints `NOT USABLE` and a
hint (for example that nothing is visible without login). `uv-pkcs11-inspect --version` reports the fork build it came from.
