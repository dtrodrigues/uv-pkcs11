...
# via the SSL_CLIENT_CERT pkcs11: URI that uv-pkcs11-inspect printed for it.
#
...
# The URI the report prints is what the install below presents, so the
# printed form is exercised end to end.
URI=$(sed -n 's/^URI: //p' <<<"$report")
if [ -z "$URI" ]; then
  echo "expected uv-pkcs11-inspect to print the identity's pkcs11: URI" >&2
  exit 1
fi
...
echo "--- with the URI uv-pkcs11-inspect printed, the install must succeed"
