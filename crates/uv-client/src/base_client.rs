...
impl ClientBuildError {
    ...
    pub fn is_user_failure(&self) -> bool {
        match self {
            // A `pkcs11:` URI that selects no identity, an unavailable
            // provider module, and a token whose objects cannot be paired are
            // all configuration the user can correct.
            Self::Credentials(_) | Self::IndexCredentials(_) | Self::Pkcs11(_) => true,
            // Failing to build a rustls configuration from an identity that
            // was already loaded is internal, not a configuration mistake.
            Self::Reqwest(_) | Self::Tls(_) => false,
        }
    }
}
