...
/// Percent-encode `value` for a `pkcs11:` URI attribute, leaving the bytes in
/// `literal` as they are.
///
/// Everything outside RFC 3986's unreserved set is encoded. RFC 7512 allows
/// more characters to stand unencoded, but encoding them keeps the URI clear
/// of the characters that delimit its own attributes and of anything a shell
/// would interpret when the URI is pasted into `SSL_CLIENT_CERT`.
fn percent_encode(value: &[u8], literal: &[u8]) -> String {
    let mut encoded = String::with_capacity(value.len());
    for &byte in value {
        if byte.is_ascii_alphanumeric()
            || matches!(byte, b'-' | b'.' | b'_' | b'~')
            || literal.contains(&byte)
        {
...
fn discover_on_token(
    ...
    for certificate in load_certificates(&session, selector)? {
        ...
        let allowed_mechanisms = key_allowed_mechanisms(&session, private_key)?;
        let supported_schemes = signing_methods(token_mechanisms, allowed_mechanisms.as_deref());
...
/// A key's non-empty `CKA_ALLOWED_MECHANISMS`, when the token reports it.
///
/// The attribute is optional in PKCS#11, and a key without it carries no
/// per-key restriction. An empty list is reported the same way: a key that
/// permitted no mechanism at all could not sign, so treating it as
/// unrestricted is no more permissive than the token's own mechanism list.
...
/// preference order.
///
/// `allowed_mechanisms` is a key's `CKA_ALLOWED_MECHANISMS`, which narrows the
/// token's mechanisms to those that key may be used with; `None` means the key
/// carries no such restriction. A mechanism the token offers but the key
/// forbids would fail at `C_SignInit`, so it must not reach the handshake as
/// an advertised scheme.
fn signing_methods(
    token_mechanisms: &[MechanismType],
    allowed_mechanisms: Option<&[MechanismType]>,
) -> Vec<(SignatureScheme, SigningMethod)> {
    SCHEME_MECHANISMS
        .iter()
        .filter_map(|entry| {
            SigningMethod::PREFERENCE
                .into_iter()
                .find(|method| {
                    let mechanism = entry.mechanism(*method);
                    token_mechanisms.contains(&mechanism)
...
fn signing_schemes(
    ...
    signing_methods(token_mechanisms, allowed_mechanisms)
        .into_iter()
        .map(|(scheme, _)| scheme)
        .collect()
}
...
mod tests {
    ...
    fn filters_unsupported_signature_schemes() {
        ...
        assert_eq!(
            signing_methods(&token_mechanisms, None),
            vec![
                (SignatureScheme::RSA_PSS_SHA256, SigningMethod::HashOnToken),
                (
                    SignatureScheme::RSA_PKCS1_SHA256,
        ...
        assert_eq!(
            signing_schemes(&[MechanismType::SHA256_RSA_PKCS], None),
            vec![SignatureScheme::RSA_PKCS1_SHA256]
        );
    }

    #[test]
    fn allowed_mechanisms_narrow_the_token_list() {
        // The token hashes and signs, but the key only permits PKCS#1 v1.5.
        let token_mechanisms = [
            MechanismType::SHA256_RSA_PKCS_PSS,
            MechanismType::SHA256_RSA_PKCS,
        ];
        ...
        // A restriction naming only mechanisms the token does not offer leaves
        // nothing, and so does one that permits nothing relevant.
        assert!(signing_schemes(&token_mechanisms, Some(&[MechanismType::RSA_PKCS])).is_empty());
        assert!(signing_schemes(&token_mechanisms, Some(&[])).is_empty());
        // The restriction applies per method: the token has both mechanisms
        // for RSA_PKCS1_SHA256, and allowing only the raw one selects it.
        let mixed = [MechanismType::SHA256_RSA_PKCS, MechanismType::RSA_PKCS];
        assert_eq!(
            signing_methods(&mixed, Some(&[MechanismType::RSA_PKCS])),
            vec![
                (
                    SignatureScheme::RSA_PKCS1_SHA512,
                    SigningMethod::HashInProcess
    ...
    #[test]
    fn percent_encodes_uri_attributes() {
        assert_eq!(percent_encode(b"MyToken-1.0_~", b""), "MyToken-1.0_~");
        assert_eq!(percent_encode(b"my token;x=1", b""), "my%20token%3bx%3d1");
        assert_eq!(percent_encode(&[0x01, 0xA2], b""), "%01%a2");
        assert_eq!(percent_encode(b"/opt/x y.so", b"/"), "/opt/x%20y.so");
        // Whatever is encoded decodes back to the bytes it came from.
        let value = (u8::MIN..=u8::MAX).collect::<Vec<_>>();
        assert_eq!(decode_bytes(&percent_encode(&value, b"")).unwrap(), value);
    }
