...
without login, reports whether exactly one usable identity is present, and
prints the `pkcs11:` URI that names each identity it finds.
...
pub fn run(
    ...
    let module_path = uri_module_path(&module);
    match identities.as_slice() {
        [identity] => {
            ...
            writeln!(
                output,
                "URI: {}",
                inspection.uri(identity, module_path.as_deref())
            )?;
            Ok(EXIT_OK)
        }
        [] => {
        ...
        many => {
            ...
            writeln!(output, "Pass one of the URIs above to choose one.")?;
            Ok(EXIT_NOT_USABLE)
        }
    }
}

/// The module path to name in a printed URI, or `None` to leave the URI
/// naming no module.
///
/// RFC 7512 requires `module-path` to be absolute, so a relative path is
/// resolved against the working directory. A bare file name is not a path at
/// all: the dynamic loader found it on its search path, which no URI can ask
/// for, so it is left out — as it is for the p11-kit proxy, which is what an
/// absent `module-path` means.
fn uri_module_path(module: &Path) -> Option<PathBuf> {
    if module.is_absolute() {
        Some(module.to_path_buf())
    } else if let Some(parent) = module.parent()
        && !parent.as_os_str().is_empty()
    {
        std::path::absolute(module).ok()
    } else {
        None
    }
}
