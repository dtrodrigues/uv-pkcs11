...
fn ambiguity_is_reported() {
    ...
    let uris = |report: &str| {
        report
            .lines()
            .filter_map(|line| line.trim().strip_prefix("URI: ").map(str::to_string))
            .collect::<Vec<_>>()
    };
    ...
    // The report names each identity it found by URI, and those URIs select
    // the identities they were printed for.
    let ambiguous = uris(&report);
    assert_eq!(ambiguous.len(), 2, "{report}");
    for uri in &ambiguous {
        let identity = Pkcs11ClientIdentity::from_uri(uri).unwrap();
        assert_eq!(common::handshake(&fixture, &identity), 1);
    }
    ...
    let selected = uris(&report);
    assert_eq!(selected.len(), 1, "{report}");
    assert!(
        selected[0].contains(";id=%02") && ambiguous.contains(&selected[0]),
        "{report}"
    );
