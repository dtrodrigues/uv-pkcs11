...
pub struct KeySpec {
    ...
    pub usage: Usage,
    /// `CKA_ALLOWED_MECHANISMS` for the private key, or `None` to leave the
    /// optional attribute unset.
    pub allowed_mechanisms: Option<Vec<MechanismType>>,
}
...
pub struct Fixture {
    pub module: PathBuf,
    pub pki: Pki,
    /// The scratch directory holding the SoftHSM configuration, its token
    /// store, and the PKI material, removed when the fixture is dropped.
    dir: PathBuf,
}

impl Drop for Fixture {
    fn drop(&mut self) {
        // A failing test's token store and certificates are worth keeping, so
        // only a passing test takes its scratch directory with it.
        if std::thread::panicking() {
            eprintln!("leaving {} behind for inspection", self.dir.display());
            return;
        }
...
fn populate_token(pkcs11: &Pkcs11, dir: &Path, pki: &Pki, spec: &TokenSpec<'_>) {
    ...
    for key in &spec.keys {
        ...
        if let Some(allowed) = &key.allowed_mechanisms {
            private_template.push(Attribute::AllowedMechanisms(allowed.clone()));
        }
