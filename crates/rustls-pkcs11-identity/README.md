...
RSA PKCS#1 v1.5 and RSA-PSS with SHA-256, SHA-384, and SHA-512, when the token
lists the corresponding `CKM_SHAxxx_RSA_PKCS[_PSS]` mechanisms; a key's
`CKA_ALLOWED_MECHANISMS` restricts the set further. Per-mechanism `CKF_SIGN`
flags and key-size ranges from `C_GetMechanismInfo` are not checked, so a
mechanism the token lists but cannot use with the selected key fails in
`C_Sign` at handshake time.
