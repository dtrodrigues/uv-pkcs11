...
transcript is hashed in process and the token pads and signs the digest. A
key's `CKA_ALLOWED_MECHANISMS`, where the provider reports it, narrows that
list to the mechanisms the key itself may be used with. Per-mechanism
`CKF_SIGN` flags and key-size ranges from `C_GetMechanismInfo` are not
checked, so a mechanism the token lists but cannot use with the selected key
fails in `C_Sign` at handshake time.
...
client uses (with no argument, the p11-kit proxy is inspected), prints the
URI that names each identity it finds, and exits 0 when exactly one identity
would be selected. The command lives in the
...
The printed URI names that identity and nothing else: it carries the token
label (or the token's serial number, when labels repeat), the `CKA_ID`, and
the certificate's `object` label when two certificates share the `CKA_ID`.
With several identities the command prints `AMBIGUOUS` and one URI per
identity, so passing one back selects it. With none it prints `NOT USABLE`
and a hint (for example that nothing is visible without login).
